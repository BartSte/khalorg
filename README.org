...
* Improvements:
  - [ ]  ~khalorg list~ support many timestamps per item. ~khalorg new~ only
    support 1 timestamp per item. It is desired that ~khalorg new~ creates an
    event for each timestamp where all events share the same UID. This is not
    possible through the ~khal new~ command.
