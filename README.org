...
*** Recurring items
    ...
    (e.g., the description). For the future, an idea could be to used 
    ~khal import~ instead of ~khal new~ for creating new event, as is explained
    at [[Improvements]].
...
** Edit
   ...
   - Do we also want to be able to change the start and stop dates?
   - Currently, only the first timestamp is processed.

   - edit
     - edit all event instances that: match the UID and the start and stop
       dates.
     - get all the fields by parsing an org file from stdin.
     - get the meetings by date.
     - filter the meetings by uid.
     - Edit all fields.
   
   I you want to edit an instance of the event, copy past the original event,
   and modify the start and stop dates to the ones you want to change and edit
   the other fields accordingly. After running the edit command only the field
   are updates for th instances that match the UID and the dates. 
   
   The khal list command will still group the events together based on their
   UID, even if their content is different. This should be fixed.
   
  - Make the get_event method accept only a uid (using Khal search), only
    timestamps (using Khal at), or both (using Khal at that is filtered on
    UID.
...
* Improvements:
  - [-] ~khal new~ for each timestamp, create an event.
  - [ ] ~khal edit~ edit field while retaining the UID.
  - [ ] ~khalorg new~ only support 1 timestamp per item. It is desired that
    ...
    the same UID. This is not possible through the ~khal new~ command.

    The best solution is to replace ~khal new~ with ~khal import~, resulting in
    a new command called ~khalorg import~ (and deprecating ~khalorg new~). This
    has the following positive side effects:
    - More complex repeat patterns are possible.    
    - The "edit_attendees" routine can be removed.
    - Multiple timestamps per org item can be processed.

      The following steps need to be performed:
    - Convert the OrgAgendaItem into and icalendar.Event object, where each
      timestamp results in 1 Event object.
    - Concatenate the Event objects into 1 ics file, where all events have the
      same UID. 
    - Feed the ics to the ~khal import~ command using ~ImportArgs.as_list~

      ICalendar files can be created using the ~icalendar~ module or the 
      ~org2ical~ module. When creating ics files, we need to ensure the UID is
      the same for the vevents. Furthermore, the appropriate RECURRENDE-ID
      needs to be added aswell.

      Alternatively, we could omit all of this and let an external tool (e.g.,
      emacs) do the org to ical conversion and feed this to ~khal import~. This
      will be more stable but gives no control over the process and adds an
      external dependency.
  - [ ] The `--until` flag can be replaced by a property in the org file.
