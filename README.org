...
** Edit
   ...
   corresponding UID through stdin to the ~khalorg edit~ command. For example, the
   org agenda item of [[New]] can be altered and used as an input for 
   ...
   * Meeting
   ...
   :UID: 123
   :LOCATION: Somewhere2
   :ORGANIZER: Someone2 (someone2@outlook.com)
   :ATTENDEES: test2@test.com, test3@test.com
   :URL: www.test2.com
   :END:
   Hello,
...
* Bugs:
  - [ ] Org item not recognized by neovim plugin when no blank line is present
    at the bottom.
  - [ ] After editing, an Overwritten Event error is raised by mycalsync. This
    happens for recuring events.
